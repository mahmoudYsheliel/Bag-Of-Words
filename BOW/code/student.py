...
def get_tiny_images(image_paths):
    ...
    result=np.zeros([len(image_paths),n*n])
...
def build_vocabulary(image_paths, vocab_size):
    ...
    for path in image_paths:
        ...
        image=np.resize(image,(resized_length,resized_length))
        fd, hog_image = hog(image, pixels_per_cell=(l,l),cells_per_block=(z, z), visualize=True,feature_vector=True)
        result.append(fd)

    result=np.array(result)
    result=(result-np.average(result))/np.std(result)
    result=np.reshape(result,(-1,z*z*9))
    Output=cluster.KMeans(n_clusters=vocab_size, init='random',n_init=10, max_iter=300, tol=1000, random_state=0).fit(result)
    return Output.cluster_centers_
...
def get_bags_of_words(image_paths):
    ...
    # TODO: Implement this function!
    result=np.zeros([len(image_paths),vocab.shape[0]])
    for i, path in enumerate(image_paths):
        image=cv2.imread(path)
        image=cv2.cvtColor(image,cv2.COLOR_RGB2GRAY)
        image=np.resize(image,(resized_length,resized_length))
        fd, hog_image = hog(image, pixels_per_cell=(l,l),cells_per_block=(z, z), visualize=True,feature_vector=True)
        fd=(fd-np.average(fd))/np.std(fd)
        fd=np.reshape(fd,(-1,z*z*9))

        distance = cdist(vocab, fd, 'euclidean')
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    y_pred = clf.predict(test_image_feats)
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    for i,closest_to_point in enumerate(closest):
        dist_sum=np.zeros(len(labels))
        
        for c in closest_to_point:
            dist_sum[np.argwhere(labels==train_labels[c])]+=1/(distances[i,c]+0.1)

        
        predicted_label= labels[np.argmax(dist_sum)]
